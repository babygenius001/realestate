	                ...
	                <table class="table table-striped table-bordered table-hover">
	                    ...
	                    <tbody>
	                    	...
	                    	@foreach($DB_table as $items)                   	
		                        <tr>
		                            <td id='{{$items->id}}'>{{++$i}}</td>
		                            <td class="box-center"><a href="{{route('NRGAdmin')}}/{{$routeLink}}/detail/{{$items->id}}">{{$items->name}}</a></td>
		                            <td class="box-center"><a href="{{route('NRGCNews_getDetail',['id'=>$items->id])}}">&laquo; List View &raquo;</a></td>	
		                            <td>{{$items->description_short}}</td>	
		                            <td class="box-img"><img src="{{asset($items->image)}}" alt="{{$items->name}}"/></td>	
		                            <td class="box-ico"><img src="{{asset($items->icon)}}" alt="{{$items->name}}"/></td>	
