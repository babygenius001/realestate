...
{{-- content --}}
		...
		<div class="panel panel-default">
	        <div class="panel-heading">
		        @if(isset($title))
	                {{$title}}{{' data - Category: '}} {{$DB_table_categories->name}} 
	            @else 
	                {{'Data'}}
	            @endif
	        </div>
	                ...
	                <table class="table table-striped table-bordered table-hover">
	                    ...
	                    <tbody>
	                    	...
	                    	@foreach($DB_table as $items)                   	
		                        <tr>
		                            <td id='{{$items->id}}'>{{++$i}}</td>
		                            <td><a href="{{route('NRGAdmin')}}/{{$routeLink}}/detail/{{$items->id}}">{{str_limit($items->title,25)}}</a></td>	
		                            <td>{{str_limit($items->summary,70)}}</td>	
