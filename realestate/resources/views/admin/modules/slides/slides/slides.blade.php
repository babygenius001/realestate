	                ...
	                <table class="table table-striped table-bordered table-hover">
	                    ...
	                    <tbody>
	                    	...
	                    	@foreach($DB_table as $items)                   	
		                        <tr>
		                            <td id='{{$items->id}}'>{{++$i}}</td>
		                            <td><a href="{{route('NRGAdmin')}}/{{$routeLink}}/detail/{{$items->id}}">{{ str_limit($items->name,25)}}</a></td>
		                            <td><a href="{{route('NRGCSlides_getItems',['id'=>$items->slides_categories_id])}}">View: {{$items->m_slides_categories->name}}</a></td>		
									<td class="box-img"><img src="{{asset($items->image)}}" alt=""></td>
